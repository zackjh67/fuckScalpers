...
const refreshInterval = argv.refresh * 1000;



function buildAlertzy(title, message, link, priority) {
  if (!alertzyAccountKey.length) return 'echo Alertzy not set up';
  return `curl -s --form-string "priority=${priority || 0}" --form-string "group=Newegg" --form-string "accountKey=${alertzyAccountKey}" --form-string "title=${machine}: ${title}" --form-string "message=${message}" ${(link && '--form-string "link=' + link + '"') || ''} https://alertzy.app/send
`
}
...
while(true) {
  try {
    ...
    if (inStock) {
      ...
      if (isNew && isNewEgg) {
        ...
        try {
          ...
          await cartButton.click();
          await exec(
            buildAlertzy('NEWEGG', `Attempting add to cart: ${destUrl}`)
          );
          ...
          await checkoutBtn.click();
          await exec(
            buildAlertzy('NEWEGG', `Attempting to check out!!: ${destUrl}`)
          );
    ...
    // wait 10 seconds then refresh
    console.log(`waiting ${argv.refresh * 1000} seconds to refresh ${destUrl}`);
    // TODO add random seconds modifier to refreshinterval second so it seems less bott-ey and predictable
    await ktimeout(refreshInterval);
    if (!inStock) {
      console.log(`${destUrl} not in stock. refreshing. ${now()}`);
