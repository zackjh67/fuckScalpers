...
function buildAlertzy(title, message, link, priority) {
    ...
    return `curl -s --form-string "priority=${priority || 0}" --form-string "group=Gamestonk" --form-string "accountKey=${alertzyAccountKey}" --form-string "title=${machineTitle}: ${title}" --form-string "message=${message}" ${(link && '--form-string "link=' + link + '"') || ''} https://alertzy.app/send
`
}

const machineTitle = argv.machine;
...
let allGpuListHref = 'https://www.gamestop.com/search/?q=rtx%203080';
let justRogHref = 'https://www.gamestop.com/search/?prefn1=webPublisher&prefv1=ASUS%20ROG&q=rtx%203080';
let gpuListHref;
const cartHref = 'https://www.gamestop.com/cart/';
if (argv.item == 0) {
    gpuListHref = 'https://www.gamestop.com/consoles-hardware';
}
if (argv.item == 1) {
    gpuListHref = justRogHref;
}
const itemSelector = '.product-tile.product-detail';
const itemLinkSelector = '.link-name';
const promoSelector = '.add-to-cart';
(async () => {
    ...
    await ktimeout(60000); //wait a minute to sign in
        ...
        try{
            ...
            if (!items.length) {
                if (lastNumItems === 0) {
                    ...
                    await exec(
                        buildAlertzy('Gamestonk ERROR', `${machineTitle} had error ${e.toString()}`, undefined, 0)
                    );
                    keepRunning = false;
                } else {
                ...
                await keach(items, async (item) => {
                    ...
                    if (!itemHref) {
                        ...
                        await exec(
                            buildAlertzy('Gamestonk ERROR (No Href)', `Error at ${now()} sry check the logs`, undefined, 0)
                        );
                    }
                    const promoContainers = await item.$$(promoSelector);
                    let inStock = false;
                    const cartBtn = promoContainers[0];
                    if (cartBtn) {
                        const containerTxt = await page.evaluate(el => el.innerText, cartBtn);
                        if (containerTxt.trim().toUpperCase() === 'ADD TO CART') {
                            console.log('in stock');
                            await exec(
                                buildAlertzy(`Gamestonk IN STOCKKK`, `instock`, `https://www.gamestop.com${itemHref}`, 2)
                            );
                            await cartBtn.click();
                            await ktimeout(1000);
                            await page.goto(cartHref);
                            let cartEmptyTxt = undefined;
                            // const cartEmptyTxt = await page.$$('.col-12.text-center');
                            if (cartEmptyTxt) {
                                console.log('CART NOT LOADED YET! REFRESHING');
                                await page.reload(gpuListHref);
                            } else {
                                // await ktimeout(1000);
                                // const checkoutBtn = (await page.$$('.checkout-btn'))[0];
                                // await checkoutBtn.click();
                                // await ktimeout(1500);
                                // const contBtn = (await page.$$('.submit-shipping'))[0]
                                // await contBtn.click();
                                // const quadPay = await page.$('#quadpay-tab');
                                // await quadPay.click();
                                // const quadPayBtn = await page.$('#quadpay-button');
                                // await quadPayBtn.click();

                                // TODO do manual stuff
                                // just make sure it doesn't refresh or do something else lol wait an hour
                                await ktimeout(600000);
                            }
                        } else if (containerTxt.trim().toUpperCase() === 'UNAVAILABLE') {
                            ...
                            if (fuck === 0) {
                                ...
                                await exec(
                                    buildAlertzy(`Gamestonk -> Unexpected button txt: ${itemDescription}`, `${machineTitle} found this in stock at ${now()}`, itemHref, 1)
                                );
                            }
                        }
                    }
                    ...
                    //     await exec(
                    //         buildAlertzy(`IN STOCK: ${itemDescription}`, `${machineTitle} found this in stock at ${now()}`, itemHref, 2)
                    //     );
                    // }
                });
                fuck = 1;
            ...
            await exec(
                buildAlertzy('Gamestonk ERROR', `Error at ${now()} sry check the logs`, undefined, 0)
            );
            keepRunning = false;
        }
    }
